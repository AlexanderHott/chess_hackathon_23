...
def test_obvious_black_win():
    obvious_black_win = (
        "rnbqkb1r/ppp1pppp/5n2/3p4/2QP4/8/PPP1PPPP/RNB1KBNR b KQkq - 0 1"
    )
...
def test_guess_move_evaluation():
    ...
    assert evaluator.guess_move_evaluation(
        board, pawn_takes
    ) > evaluator.guess_move_evaluation(board, knight_takes)
    assert evaluator.guess_move_evaluation(
        board, knight_takes
    ) > evaluator.guess_move_evaluation(board, none_takes)
    # test pawn avoiding
    board = chess.Board(
    ...
    with open("../data/test_positions.txt", "r") as games:
        for count, line in enumerate(games):
            ...
            if count % 2 == 1:
                ...
                evaluator.search(
                    position, depth=2, debug_counts=True, guess_move_order=False
                )
                ...
                start = time.perf_counter()
                evaluator.search(
                    position, depth=2, debug_counts=True, guess_move_order=True
                )
...
def test_check_all_captures():
    board = chess.Board(
        "r4rk1/1p2npb1/pqn1p2p/1B1pPbp1/Q2P4/1PN1BN2/P4PPP/2R1K2R b K - 0 14"
    )
    evaluator.search(
        board, 0, search_captures=False, search_checks=False, debug_counts=True
    )
    ...
    evaluator.reset_debug_vars()
    evaluator.search(
        board, 0, search_captures=True, search_checks=False, debug_counts=True
    )
    ...
    evaluator.reset_debug_vars()
    evaluator.search(
        board, 0, search_captures=True, search_checks=True, debug_counts=True
    )
...
def play_two_bots(
    new: bot.Bot, old: bot.Bot, number_games: int = 1, fen: str = chess.STARTING_FEN
) -> tuple[int, int, int]:
